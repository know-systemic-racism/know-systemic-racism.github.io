...
viewof fe_rows = Inputs.table(fe_search, {
  ...
  format: {
    ...
    ' Date of injury resulting in death (month/day/year)': x => new Date(x)
  },
})
```

```{ojs}
tidy = transpose(fe_data).map(d => ({name: 'Name', date: new Date(d[' Date of injury resulting in death (month/day/year)']).getFullYear(), count: 1, race: 'Race'}))
```

```{ojs}
// fe_data_2 = d3.autoType(transpose(fe_data))
// Plot.rectY(transpose(fe_data), 
//   Plot.binX(
//     {y: "count"}, 
//     {x: ' Date of injury resulting in death (month/day/year)', fill: 'Race'}
//   )).plot({
//     marks: [
//       Plot.frame(),
//     ]
...
Plot.barY(tidy, {x: 'date', y: 'count', fill: 'race'}).plot()
```
